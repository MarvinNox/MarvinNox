# Hi there, I'm Denys 👋

🎯 **Full-Stack Web Developer** from **Ukraine**  
I build scalable, dynamic web applications using modern JavaScript tools and frameworks.  
Passionate about clean code, efficient architecture, and bringing great ideas to life through technology.

---

### 🛠️ Tech Stack

- **Frontend:** HTML, CSS, JavaScript, TS, React, Next.js, Sass / SCSS, Tailwind, shadcn/ui,
- **Backend:** Node.js, Express, REST APIs
- **Databases:** MongoDB, PostgreSQL
- **Tools & Workflow:** Git, Vercel, Linux, VSCode, Zustand, TanStack Query, Postman

---

### 💡 About Me

- 💻 Love solving complex UI/UX and API integration challenges
- 🧩 Always learning and experimenting with new web technologies
- 🌍 Open for collaboration and interesting projects

---

### 📫 How to reach me

[![GitHub](https://img.shields.io/badge/GitHub-12100E?style=flat-square&logo=github&logoColor=white)](https://github.com/MarvinNox)  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/denys--tkachenko)

---

## 📊 GitHub Stats

<div align="center">
  ![Top Langs](https://gprm.itsvg.in/api/top-langs?username=MarvinNox&layout=compact&theme=radical)
  ![Stats](https://gprm.itsvg.in/api?username=MarvinNox&show_icons=true&theme=radical)
  ![Streak](https://github-readme-streak-stats-eight.vercel.app/?user=MarvinNox&theme=radical)
</div>
---

⭐️ _“Code. Learn. Repeat.”_
